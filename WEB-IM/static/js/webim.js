...
var send = function () {
	...
	var fI = $('#fileInput'); 
	var fileInfo = Easemob.im.Helper.getFileUrl(fI); 
	...
	if (!Easemob.im.Helper.isCanUploadFileAsync || filetype.toLowerCase() in pictype) {
		var opt = {
			...
			onFileUploadComplete : function(data) {
                ...
                var viewer = new Viewer(document.getElementById("dowebok"+num));
			},
		...
		var opt = {
			...
			/*	var messageContent = "发送文件" + (filename || data.filename);
				...
				if ( Easemob.im.Helper.isCanUploadFileAsync && file && file.files) {
					...
					filename = data.filename || '';
					var img = document.createElement("a");
					img.href = data.uri + '/' + data.entities[0].uuid;
...
//显示聊天记录的统一处理方法
var appendMsg = function(who, contact, message, onlyPrompt,url) {
       console.log(message);
	...
	for (var i = 0; i < flg; i++) {
		...
		if (type == "emotion") {
			...
			var ele1 =$("<div style='min-width:150px;text-align:right;border-bottom: 1px solid rgba(0, 0, 0, 0.15);line-height:30px;margin-bottom:4px;'>" +'文件'+ "</div>"+"<br>");
			var ele = $("<p></p>"+"<br>");
			    //aaaa= $("<a target='_blank' href="+data+">点击下载</a>")
			ele.attr("class", "chat-content-p3");
		    //var  spans=$("<span>"+url+"<span>"+"<br>");
			//aaaa.attr("class", "chat-content-p3");
			ele.get(0).appendChild(ele1.get(0));
			//ele.get(0).appendChild(spans.get(0));
            lineDiv.appendChild(ele.get(0));
            //ele.get(0).appendChild(aaaa.get(0));
            var zip=$("<div>").html("<div class='file_undefined'><span class='icon icon-mid'><span class='icon-file'></span></span><div style='display:inline-block;margin-left:10px;'><span>"+url+"</span><br><a href='"+data+"'>点击下载</a></div></div>");
            zip.attr("class", "chat-content-p3");
            ele.get(0).appendChild(zip.get(0));
	...
	if ( onlyPrompt ) {
		...
		lineDiv.style.paddingRight =10+"px";
	} else {
		lineDiv.style.textAlign = "left";
		lineDiv.style.paddingLeft =10+"px";
	}
	var create = false;
